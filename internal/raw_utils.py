...
def load_raw_dataset(split: utils.DataSplit,
  ...
  if not train_mode:
    images_rgb = []
    new_h, new_w = int(3024/n_downsample), int(4032/n_downsample)
    for image_name in image_names:
      image_rgb = Image.open(os.path.join(data_dir, "images", image_name))
      image_rgb = image_rgb.resize((new_w, new_h))
      images_rgb.append(image_rgb)
    images_rgb = np.stack(images_rgb,axis=0)/255.
  ...
  if not train_mode:
    return images, meta, testscene, images_rgb
  else:
