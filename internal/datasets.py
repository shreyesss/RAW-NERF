...
class Dataset(threading.Thread, metaclass=abc.ABCMeta):
  ...
  def __init__(self,
    ...
    self.width: int = None
    self.images_rgb: np.ndarray = None 
  ...
  def _make_ray_batch(self,
    ...
    if not self.render_path:
      ...
      if not self.train_mode:
        batch['rgb_jpeg'] = self.images_rgb[cam_idx, ...]
    if self._load_disps:
  ...
  def _next_train(self) -> utils.Batch:
    ...
    # print(pix_x_int, patch_dx_int, lower_border, upper_border)
  ...
  def _load_renderings(self, config):
    ...
    if config.rawnerf_mode:
      ...
      if config.train_mode:
        ...
        images, metadata, raw_testscene, images_rgb = raw_utils.load_raw_dataset(
            self.split,
            self.data_dir,
            image_names,
            config.exposure_percentile,
    ...
    if config.llff_use_all_images_for_training or raw_testscene:
      ...
      with open(train_file_path, 'r') as file:
          ...
          train_file.sort()
      test_file_path = os.path.join(self.data_dir, "test.txt")
      with open(test_file_path, 'r') as file:
          test_file = file.read().splitlines()
          test_file.sort()
      split_indices = {
          utils.DataSplit.TEST: [idx for idx in all_indices if image_names[idx].split(".")[0] in test_file],
          utils.DataSplit.TRAIN: [idx for idx in all_indices if image_names[idx].split(".")[0] in train_file],
      }
    # print(image_names)
    # print([idx for idx in all_indices if image_names[idx].split(".")[0] in test_file])
    # print(test_file)
    # print(self.data_dir)
    # import sys
    # sys.exit()
